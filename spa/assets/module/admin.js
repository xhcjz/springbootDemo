...
layui.define(['layer', 'setter', 'layRouter'], function (exports) {
        ...
        admin.ajax({
            ...
            success: function (res) {
                ...
                if (tpl === true || tpl === 'true') {
                    ...
                    // 模板里面有动态模板处理
                    var $html = $('<div>' + res.replace(/src=/g, 'e_src=') + '</div>'), tplAll = {};
    ...
    admin.hasPerm = function (r) {
        ...
        if (auth) for (var i = 0; i < auth.length; i++) if (r == auth[i]) return true;
        return false;
    };
    ...
    admin.events = {
        ...
        theme: function () {
            ...
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-theme', url: option.url || 'components/tpl/theme.html'
        ...
        note: function () {
            var option = admin.util.deepClone($(this).data());
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-note', url: option.url || 'components/tpl/note.html'
            }, admin.parseLayerOption(option)));
        },
        /* 打开消息 */
        message: function () {
            var option = admin.util.deepClone($(this).data());
            admin.strToWin(option.window).layui.admin.popupRight($.extend({
                id: 'layer-notice', url: option.url || 'components/tpl/message.html'
            }, admin.parseLayerOption(option)));
        },
        /* 打开修改密码弹窗 */
        psw: function () {
    ...
    admin.util = {
        ...
        scrollTop: function (elem) {
            if (!elem) {
                var $layBody = $('.layui-layout-admin>.layui-body');
